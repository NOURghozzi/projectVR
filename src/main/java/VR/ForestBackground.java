...
public class ForestBackground {
    ...
    private void drawCircle(GL2 gl, float centerX, float centerY, float centerZ, float radius, int slices) {
        gl.glBegin(GL2.GL_TRIANGLE_FAN);
        gl.glVertex3f(centerX, centerY, centerZ); // center point
        for (int i = 0; i <= slices; i++) {
            double angle = 2 * Math.PI * i / slices;
            float x = (float)(centerX + Math.cos(angle) * radius);
            float z = (float)(centerZ + Math.sin(angle) * radius);
            float y = centerY + (float)(Math.sin(angle) * (radius * 0.8)); // **Ajout bombé sur Y**
            gl.glVertex3f(x, y, z);
        }
        gl.glEnd();
    }
    private void drawCircle2(GL2 gl, float centerX, float centerY, float centerZ, float radius, int slices) {
        gl.glBegin(GL2.GL_TRIANGLE_FAN);
        gl.glVertex3f(centerX, centerY, centerZ); // center point
        for (int i = 0; i <= slices; i++) {
            double angle = 2 * Math.PI * i / slices;
            float x = (float)(centerX + Math.cos(angle) * radius);
            float z = (float)(centerZ + Math.sin(angle) * radius);
            float y = centerY + (float)(Math.sin(angle) * (radius * 0.3)); // **Ajout bombé sur Y**
            gl.glVertex3f(x, y, z);
        }
        gl.glEnd();
    }
    ...
    private void drawTree(GL2 gl, float x, float y, float z, int seed) {
        ...
        float height = 6.5f + (seed % 100) / 150f;
        ...
        // Foliage (draw a circle)
        // Foliage (draw the first circle with green color)
        gl.glColor3f(0f, green, 0.2f); // Green color for foliage
        drawCircle(gl, 0f, height + 0.1f, 0f, height * 0.6f, 20);

        // Foliage (draw the cone with a different color, e.g., dark green)
        gl.glColor3f(0f, green * 1.5f, 0f); // Darker green for the cone
